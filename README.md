...
Access is enforced by **Firestore security rules** plus a **collection-group index** on `shares.email`. If you haven't deployed them yet:
...
3. Deploy the rules **and** the index: `firebase deploy --only firestore:rules,indexes`

- The rules live in **`firestore.rules`** and the index config in **`firestore.indexes.json`** (both wired up in **`firebase.json`**).
- Until they're deployed, the app shows a warning instead of shared debts — rules missing = `permission-denied`, index missing = `failed-precondition` ("requires an index"). Press **Retry** after deploying; a subscription won't recover on its own.
